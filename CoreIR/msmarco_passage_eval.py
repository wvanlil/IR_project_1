...
def load_reference_from_stream(f):
    ...
    for l in f:
        try:
            ...
            qids_to_relevant_passageids[qid].append((int(l[2]), int(l[3])))
        except:
    ...
    for qid in qids_to_ranked_candidate_passages:
        if qid in qids_to_relevant_passageids:
            ...
            for i in range(0,MaxMRRRank):
                #for j in range(0, 1):
                if ((candidate_pid[i], 3) in target_pid) or ((candidate_pid[i], 2) in target_pid) or ((candidate_pid[i], 1) in target_pid):
                    # MRR += 1/(i + 1)
                    MRR += 1 / (i + 1)
                    ranking.pop()
                    ranking.append(i+1)
                    #print("Query " + str(qid) + " rank " + str(i) + " from passage " + str(candidate_pid[i]))
                    print(str(qid) + " " + str(candidate_pid[i]) + " " + str(i))
                    break
    if len(ranking) == 0:
